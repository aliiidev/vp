...
class Users_Model extends Model
{
    ...
    public function submit_index($data_sybmit)
    {
        ...
        if ($queryCount < 1) {
            $sql = "INSERT INTO `users` (`id`,`username`, `password`, `email`, `mobile`, `multiuser`, `startdate`, `finishdate`, `finishdate_one_connect`, `enable`, `traffic`, `referral`, `info`) VALUES (NULL,?,?,?,?,?,?,?,?,?,?,?,?)";
            $stmt = $this->db->prepare($sql);
            $stmt->execute(array($data_sybmit['username'], $password, $data_sybmit['email'], $data_sybmit['mobile'], $data_sybmit['multiuser'], $data_sybmit['startdate'], $data_sybmit['finishdate'], $data_sybmit['finishdate_one_connect'], $data_sybmit['enable'], $data_sybmit['traffic'], $data_sybmit['referral'], $data_sybmit['info']));
            if ($stmt) {
            ...
            <div class="p-4 mb-2" style="position: fixed;z-index: 9999;left: 0;">
              <div class="toast fade show" role="alert" aria-live="assertive" aria-atomic="true">
                ...
                <div class="toast-body">نام کاربری تکراری می باشد</div>
              </div>
            </div>';
        }
    }
