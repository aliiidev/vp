...
class Settings extends Controller
{
    ...
    function update_settings()
    {
        ...
        if (isset($_POST['upbackup'])) {
            ...
            <div class="p-4 mb-2" style="position: fixed;z-index: 9999;left: 0;">
              <div class="toast fade show" role="alert" aria-live="assertive" aria-atomic="true">
                ...
                <div class="toast-body">تنها فایل SQL پشتیبانی می شود</div>
              </div>
            </div>';
        ...
        if (isset($_POST['blockip'])) {
            $list_block = htmlentities($_POST['list_block']);
            $data_sybmit = array(
                'list_block' => $list_block
            );
            $this->model->submit_blockip($data_sybmit);
        }
        ...
        if (isset($_POST['active_blockip'])) {
            ...
            $linecount = 0;
            $bash_text = "#!/bin/bash \n";
            for ($iplist = fopen("/var/www/html/cp/Libs/iplist.txt", "r"); $line = fgets($iplist); $linecount++) {
                $line = preg_replace("/\\s+/", "", $line);
                $bash_text = $bash_text . "sudo iptables -A OUTPUT -d " . $line . " -j DROP \n";
            }
            fclose($iplist);
            file_put_contents("/var/www/html/cp/Libs/sh/banip.sh", $bash_text);
            shell_exec("bash /var/www/html/cp/Libs/sh/banip.sh");
        }
